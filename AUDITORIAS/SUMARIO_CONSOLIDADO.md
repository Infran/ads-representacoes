...
| `ThemeProvider`/tokens | ✅ **tema light/dark + biblioteca atômica `src/ui`** (U2.1); hex **113 → 43** (U2.2); **dark mode ativo** (U3.2); **dashboard hero+charts** (U3.1); **estados+responsividade** (U2.3/U2.4) | 1 tema light/dark · hex ≈ 0 fora de `tokens.ts` | UI · Ondas 2 ✅ / 4 ✅ |
...
| Suíte de testes | ✅ **72 testes** (60 unit/characterization jsdom + 12 rules no emulador) | unit + characterization + rules | EST F1 + SEG S3.1 · Ondas 2–3 ✅ |
...
**Status geral:** 🟢 **Ondas 0–4 concluídas no código** (2026-07-11). Onda 4 fechada: ✅ EST F3 (God Components), ✅ UI U2.1/U2.2 (átomos + hex **113→43**), ✅ UI U2.3/U2.4 (estados + responsividade), ✅ UI U3.1 (dashboard hero KPI + charts lazy) / U3.2 (dark mode). **Lint 7** (2 erros + 5 warnings pré-existentes com dono em EST/SEG/UI). **72 testes verdes** (60 jsdom + 12 emulador). **Falta:** Onda 5 (refino/governança).
**Próximo passo:** **Onda 5** — começar por **EST F4.5** (logger por ambiente → resolve SEG-11/PERF-12; hoje só o `esbuild.drop` de prod, dev ainda loga) ou **EST F4.1/UI U3.4** (tirar `Swal` de dentro do `useBudgetForm` e migrar confirms restantes p/ o átomo `Feedback` já pronto). Demais itens da Onda 5: UI U3.3 (WCAG AA), U3.5 (`no-color-literals`), U3.6 (Storybook opcional), PERF P2 (budget summary após ADR EST F4.6; contextos por entidade). **Pendências carregadas (não bloqueiam):** rewire do boot da Home p/ ler só 5 (P0.3/P1.2 — habilitado por U3.1 mas depende de PERF P2.1 "budget summary"); `tabela_ncm.json` sob demanda (chunk `Products` ~2,9 MB); **gate de CI no `deploy.yaml` = decisão do usuário** (SEG-09-rev, branches cruzadas); tokenizar gradiente do Login + categóricas do search; regra `no-color-literals` (U3.5).
