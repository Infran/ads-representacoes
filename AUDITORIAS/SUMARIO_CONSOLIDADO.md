...
**Status geral:** 🟢 **Ondas 0–4 concluídas** + **Onda 5 em andamento** (2026-07-11). **EST F4 100% concluída** (F4.1–F4.7); **UI U3.4 + U3.5** feitas. **Lint 7** (2 erros + 5 warnings pré-existentes com dono em EST/SEG/UI). **73 testes verdes** (61 jsdom + 12 emulador). **Falta na Onda 5:** UI U3.3 (WCAG AA), U3.6 (Storybook opcional); PERF P2.1/P2.2.
**Próximo passo:** **PERF P2.1** ("budget summary" — desbloqueado pelo ADR F4.6: projetar um resumo leve de orçamento p/ as listas/Home, habilitando o rewire do boot p/ ler 5 e não N) **ou** **UI U3.3** (WCAG AA — axe/Lighthouse nos fluxos principais; parte estática eu faço, o "score" pede o app rodando). U3.6 (Storybook) é opcional; PERF P2.2 (contextos por entidade) depende de profiling. **Pendências carregadas (não bloqueiam):** rewire do boot da Home p/ ler só 5 (P0.3/P1.2 — agora só depende de PERF P2.1); `tabela_ncm.json` sob demanda (chunk `Products` ~2,9 MB); **gate de CI no `deploy.yaml` = decisão do usuário** (SEG-09-rev, branches cruzadas); tokenizar gradiente do Login + categóricas do search (allowlistados por U3.5, não bloqueiam).
