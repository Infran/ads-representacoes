...
- [x] `npm run lint` com `--max-warnings 0`: **0 problemas** (2026-07-12). Todo o código morto que era dono da F0 foi eliminado (unused imports/vars: 21 → 0). Os 7 problemas pré-existentes remanescentes (`no-explicit-any`/`ban-types {}` em `PageHeader`/`ContextAuth`, `react-refresh/only-export-components` em `DataContext`/`ContextAuth`/`LayoutContext`/`BudgetPdf`, `exhaustive-deps` em `ContextAuth`) foram fechados na "correção rápida" pós-Onda 5: `PageHeader` passou a usar o default genérico de `SvgIconTypeMap` (sem `{}` literal); `ContextAuth` trocou `catch (error: any)` por narrowing tipado (padrão já usado em `Login.tsx`) e estabilizou `logout`/`scheduleAutoLogout` via `useCallback` para o `useEffect` poder listar a dependência sem reexecutar a cada render; `DataContext`/`LayoutContext`/`BudgetPdf` ganharam `eslint-disable-next-line react-refresh/only-export-components` (mesmo padrão já usado em `ColorModeContext`, arquitetural — hook/utilitário coexistindo com componente/contexto no mesmo módulo). Detalhe em `PLANO_EXECUCAO_SEGURANCA.md` (log 2026-07-12).
